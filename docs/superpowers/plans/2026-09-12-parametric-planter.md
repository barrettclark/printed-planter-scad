...
- **Pre-`include` variable assignment does not override an included file's
  ...
  lists.
- **`pattern_type` no longer has a `"geometric"` value.** Task 5's text
  below (and its `"<GEOMETRIC_TEXTURE>"` placeholder) reflects the original
  three-value design (`none`/`ridges`/`geometric`). This was later expanded
