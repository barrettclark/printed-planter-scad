...
| Parameter | Description | Default |
|-----------|-------------|---------|
| `pattern_type` | Decoration style: `"none"`, `"ridges"`, or a geometric tile pattern — `"diamonds"`, `"hex_grid"`, `"pyramids"`, `"bricks"`, `"checkers"`, `"dots"`, `"cubes"`, `"tri_grid"`, `"teardrop"`, `"tumbling_cubes"`, `"intertwine"`, `"islamic_star"`, `"tetrakis_square"`, `"kisrhombille"`, `"triakis_triangular"`, `"rhombille"`, `"cairo_pentagonal"` | `"ridges"` |
...
Six of the `pattern_type` values — `"teardrop"`, `"tumbling_cubes"`, `"intertwine"`, `"islamic_star"`, `"rhombille"` and `"cairo_pentagonal"` — are *interlocking* patterns: their motifs deliberately cross the tile boundary so that adjacent repeats join into one continuous design rather than sitting in visible boxes. The [gallery](docs/gallery.md#pattern-comparison) shows what each one looks like; the notes below cover their rendering quirks.
...
`"intertwine"` is by far the most expensive of the five to render: roughly 2-3 minutes and a ~25MB STL at the shipped defaults on a modern laptop, versus well under a minute for `"tumbling_cubes"`/`"islamic_star"`/`"rhombille"`/`"cairo_pentagonal"`. A silent console for a couple of minutes on `"intertwine"` is normal — it is not a hang, and it is not the CGAL failure above (which happens quickly, not after a minute or more).
