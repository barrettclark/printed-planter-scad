...
| Parameter | Description | Default |
|-----------|-------------|---------|
| `pattern_type` | Decoration style: `"none"`, `"ridges"`, or a geometric tile pattern — `"diamonds"`, `"hex_grid"`, `"pyramids"`, `"bricks"`, `"checkers"`, `"dots"`, `"cubes"`, `"tri_grid"`, `"teardrop"`, `"tumbling_cubes"`, `"intertwine"`, `"islamic_star"`, `"tetrakis_square"`, `"kisrhombille"`, `"triakis_triangular"`, `"rhombille"`, `"cairo_pentagonal"`, `"floret_pentagonal"`, `"deltoidal_trihexagonal"` | `"ridges"` |
...
`"deltoidal_trihexagonal"` (the deltoidal trihexagonal tiling — 14 kite plateaus per unit tile) joins that same mild bucket, despite being built from the same general class of large flat plateau construction as `"tumbling_cubes"`/`"rhombille"`/`"cairo_pentagonal"`/`"floret_pentagonal"` above, not the "kis" family's small-triangle-facet safety. A full sweep of the real assembly found **zero CGAL aborts anywhere tested**: every `pattern_repeat` from 3 through 16 at `smoothness=24`, and every `smoothness` in 24/40/60/80/100 at the shipped `pattern_repeat=16`, in **both** relief modes — 36 combinations, all clean. This is a genuinely different result from every other large-flat-plateau pattern in this project, all of which found at least one real abort somewhere in an equivalent sweep. CI pins both a reduced value (`pattern_repeat=4`, the same cheap low value the "kis" family uses — with every tested value clean, there is no "isolated clean value between two aborts" trap to avoid here) and the shipped defaults (`pattern_repeat=16`, `smoothness=60`). Everything above is from a local macOS OpenSCAD 2021.01 build and has **not** yet been re-verified against GitHub Actions' Ubuntu build — the `"intertwine"` note above is the standing reminder that local-clean is necessary but not sufficient; if either CI check ever fails, treat it as the authoritative re-measurement rather than assuming the local sweep still holds.
...
`"intertwine"` is by far the most expensive of the seven to render: roughly 2-3 minutes and a ~25MB STL at the shipped defaults on a modern laptop, versus well under a minute for `"tumbling_cubes"`/`"islamic_star"`/`"rhombille"`/`"cairo_pentagonal"`/`"floret_pentagonal"`/`"deltoidal_trihexagonal"`. A silent console for a couple of minutes on `"intertwine"` is normal — it is not a hang, and it is not the CGAL failure above (which happens quickly, not after a minute or more).
