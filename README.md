...
| Parameter | Description | Default |
|-----------|-------------|---------|
| `pattern_type` | Decoration style: `"none"`, `"ridges"`, or a geometric tile pattern — `"diamonds"`, `"hex_grid"`, `"pyramids"`, `"bricks"`, `"checkers"`, `"dots"`, `"cubes"`, `"tri_grid"`, `"teardrop"`, `"tumbling_cubes"`, `"intertwine"`, `"islamic_star"`, `"tetrakis_square"`, `"kisrhombille"`, `"triakis_triangular"`, `"rhombille"` | `"ridges"` |
...
`"intertwine"` is by far the most expensive of the four to render: roughly 2-3 minutes and a ~25MB STL at the shipped defaults on a modern laptop, versus well under a minute for `"tumbling_cubes"`/`"islamic_star"`/`"rhombille"`. A silent console for a couple of minutes on `"intertwine"` is normal — it is not a hang, and it is not the CGAL failure above (which happens quickly, not after a minute or more).
