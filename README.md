...
Note: tiles come out about **3.75× wider than tall** on the pot. `tex_reps` is set to `[pattern_repeat, pattern_repeat]`, which asks for the same repeat count around the circumference as up the height without accounting for the pot's proportions — at defaults that is 16 tiles around the decorated wall's ~517mm circumference (radius ~82.3mm, per `planter.scad`'s own peak-aware cone calculation) and 16 up a 138mm wall, so each tile is about 32.3 × 8.6mm. `"ridges"` and `"bricks"` are unaffected in practice; large-motif patterns like `"islamic_star"` are squashed enough to lose the motif. Raising `pattern_repeat` shortens the tiles vertically (and multiplies them horizontally). See [the gallery's full-pot section](docs/gallery.md#full-pot-examples) for side-by-side evidence.
