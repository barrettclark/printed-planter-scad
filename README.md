...
**See the [pattern gallery](docs/gallery.md) for a rendered example of every `pattern_type` in both relief modes**, plus full-pot renders, the three insert presets and both outer shape modes. The rest of this section covers the behaviour the pictures can't show.
...
Note: tiles come out about **3.2× wider than tall** on the pot. `tex_reps` is set to `[pattern_repeat, pattern_repeat]`, which asks for the same repeat count around the circumference as up the height without accounting for the pot's proportions — at defaults that is 16 tiles around ~446mm and 16 up a 138mm wall, so each tile is 27.9 × 8.6mm. `"ridges"` and `"bricks"` are unaffected in practice; large-motif patterns like `"islamic_star"` are squashed enough to lose the motif. Raising `pattern_repeat` shortens the tiles vertically (and multiplies them horizontally). See [the gallery's full-pot section](docs/gallery.md#full-pot-examples) for side-by-side evidence.
